...
if  phase == 'test':
    ...
    with torch.no_grad():   
        ...
        for i, data in enumerate(tqdm(loader_test, 0)):
            # forward pass
            input1 = data['img_tensor_8'].to(device)
            input2 = data['img_tensor_4'].to(device)
            ...
            # mean
            # output = (prob1 + prob2 + prob3) / 3
            # print(output.shape) # (b, 1)
            # output = (prob1 + prob2 + prob3 + prob4 + prob5) / 5
            output = (prob1 + prob2 + prob3 + prob4) / 4
