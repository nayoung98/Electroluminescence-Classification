...
def min_max_3(prob1, prob2, prob3):
    cnt = 0
    dit_list = []
...
def min_max_4(prob1, prob2, prob3, prob4):
    cnt = 0
    dit_list = []
    for i in range(prob1.shape[0]):
        if prob1[i] > 0.5:
...
def min_max_5(prob1, prob2, prob3, prob4, prob5):
    cnt = 0
    dit_list = []
    for i in range(prob1.shape[0]):
        if prob1[i] > 0.5:
...
if  phase == 'test':
    ...
    with torch.no_grad():   
        ...
        for i, data in enumerate(tqdm(loader_test, 0)):
            ...
            # dit_list = min_max_3(prob1, prob2, prob3)
            # dit_list = min_max_5(prob1, prob2, prob3, prob4, prob5)
            dit_list = min_max_4(prob1, prob2, prob3, prob4)
            
            # for i in range(batch_size):
            #     if dit_list[i] == 'max':
            #         max_value1, _ = torch.max(torch.stack([prob1[0], prob2[0], prob3[0]]), dim=0)
            #         max_value2, _ = torch.max(torch.stack([prob1[1], prob2[1], prob3[1]]), dim=0)
            #         max_value3, _ = torch.max(torch.stack([prob1[2], prob2[2], prob3[2]]), dim=0)
            #         max_value4, _ = torch.max(torch.stack([prob1[3], prob2[3], prob3[3]]), dim=0)
                    
            #         output = torch.cat([max_value1, max_value2, max_value3, max_value4], dim=0) # (b)
            #     else: # min
            #         min_value1, _ = torch.min(torch.stack([prob1[0], prob2[0], prob3[0]]), dim=0)
            #         min_value2, _ = torch.min(torch.stack([prob1[1], prob2[1], prob3[1]]), dim=0)
            #         min_value3, _ = torch.min(torch.stack([prob1[2], prob2[2], prob3[2]]), dim=0)
            #         min_value4, _ = torch.min(torch.stack([prob1[3], prob2[3], prob3[3]]), dim=0)
                    
            #         output = torch.cat([min_value1, min_value2, min_value3, min_value4], dim=0) # (b)
            
            # for i in range(batch_size):
            #     if dit_list[i] == 'max':
            #         max_value1, _ = torch.max(torch.stack([prob1[0], prob2[0], prob3[0], prob4[0], prob5[0]]), dim=0)
            #         max_value2, _ = torch.max(torch.stack([prob1[1], prob2[1], prob3[1], prob4[1], prob5[1]]), dim=0)
            #         max_value3, _ = torch.max(torch.stack([prob1[2], prob2[2], prob3[2], prob4[2], prob5[2]]), dim=0)
            #         max_value4, _ = torch.max(torch.stack([prob1[3], prob2[3], prob3[3], prob4[3], prob5[3]]), dim=0)
                    
            #         output = torch.cat([max_value1, max_value2, max_value3, max_value4], dim=0) # (b)
            #     else: # min
            #         min_value1, _ = torch.min(torch.stack([prob1[0], prob2[0], prob3[0], prob4[0], prob5[0]]), dim=0)
            #         min_value2, _ = torch.min(torch.stack([prob1[1], prob2[1], prob3[1], prob4[1], prob5[1]]), dim=0)
            #         min_value3, _ = torch.min(torch.stack([prob1[2], prob2[2], prob3[2], prob4[2], prob5[2]]), dim=0)
            #         min_value4, _ = torch.min(torch.stack([prob1[3], prob2[3], prob3[3], prob4[3], prob5[3]]), dim=0)
            ...
            for i in range(batch_size):
                if dit_list[i] == 'max':
                    ...
                    min_value1, _ = torch.min(torch.stack([prob1[0], prob2[0], prob3[0], prob4[0]]), dim=0)
                    min_value2, _ = torch.min(torch.stack([prob1[1], prob2[1], prob3[1], prob4[1]]), dim=0)
                    min_value3, _ = torch.min(torch.stack([prob1[2], prob2[2], prob3[2], prob4[2]]), dim=0)
                    min_value4, _ = torch.min(torch.stack([prob1[3], prob2[3], prob3[3], prob4[3]]), dim=0)
