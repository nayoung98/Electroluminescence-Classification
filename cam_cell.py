...
if  phase == 'test':
    ...
    for i, data in enumerate(tqdm(loader_test, 0)):
        ...
        for cell in range(cells):
            single_input = input[cell].unsqueeze(0) 

            targets = None
            single_input.requires_grad = True
            
            el_cam = cam(input_tensor=single_input, targets=targets)
            el_cam = el_cam[0, :]
            ...
            input_clone = single_input[0].permute(1, 2, 0)
        ...
        for j in range(rows):
            row_images.append(np.concatenate(all_visualizations[j*cols:(j+1)*cols], axis=1))
        combined_visualization = np.concatenate(row_images, axis=0)
   
        cv2.imwrite(os.path.join(save_dir, f"grad_cam_{img_name}.jpg"), combined_visualization)
